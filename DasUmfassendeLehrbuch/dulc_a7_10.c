...
int count_spruenge(int *uebergabe_schachbrett)
{
  int aufrufe = 0;
  for(int zeilen = 0; zeilen < 8; zeilen++)
  {
    for(int spalten = 0; spalten < 8; spalten++)
    {
      if (uebergabe_schachbrett[spalten+8*zeilen] >= aufrufe)
      {
        aufrufe = uebergabe_schachbrett[spalten+8*zeilen];
      }
      else
      {
        /* Ist die Zahl kleiner als die aktuell ermittelte größte Zahlm tue nichts */
      }
    }
  }
  return aufrufe;
...
int springer_problem(int *schachbrett, int akt_zeile, int akt_spalte, int ziel_zeile, int ziel_spalte, int anzahl_aufruf, int erlaubte_aufrufe)
{
  if (  (akt_zeile  <= 7)
      &&(akt_zeile  >= 0)
      &&(akt_spalte <= 7)
      ...
      &&(anzahl_aufruf <= erlaubte_aufrufe)
     )
  {
    ...
    else
    {
      ...
      {
        if (springer_problem((int*) schachbrett, akt_zeile+springer_zuege[zuege][0], akt_spalte+springer_zuege[zuege][1], ziel_zeile, ziel_spalte, anzahl_aufruf+1, erlaubte_aufrufe) == 1)
        {
          return 1;
        }
        else
  ...
  while (prog_start)
  {
    ...
    switch(abfrage)
    {
      case 1:
      {
        ...
        springer_problem((int*)schachbrett, startfeld_zeile-1, startfeld_spalte-1, zielfeld_zeile-1, zielfeld_spalte-1, 0, erlaubte_aufrufe);
        ...
        for(int test_start_zeile = 1; test_start_zeile <= 8; test_start_zeile++)
        {
          for (int test_start_spalte = 1; test_start_spalte <= 8; test_start_spalte++)
          {
            for (int test_ziel_zeile = 1; test_ziel_zeile <= 8; test_ziel_zeile++)
            {
              ...
              {
                init_schachbrett((int*)schachbrett);
                springer_problem((int*)schachbrett, test_start_zeile-1, test_start_spalte-1, test_ziel_zeile-1, test_ziel_spalte-1, 0, erlaubte_aufrufe);
                printf("Test: %d mit Startfeld: [%d][%d] und Zielfeld: [%d][%d] in %d Zügen\n", test_anzahl, test_start_zeile, test_start_spalte, test_ziel_zeile, test_ziel_spalte, count_spruenge((int*)schachbrett));
                //print_schachbrett((int*)schachbrett);
                test_anzahl++;
              }
            }
